...
import ch.datascience.graph.service.security.PublicKeyReader
...
class JWTVerifierProvider @Inject() (
  ...
  private[this] lazy val verifier: JWTVerifier = {
    ...
    JWT.require( algorithm ).withAudience( configuration.getString( "authorization.audience" ).get ).build()
  }
